...
export async function register(req, res, next) {
  try {
    ...
    if (!name || !email || !password) throw new CustomError("모든 필드를 입력하세요.", 400);
    ...
    res.status(201).json({ success: true, data: { user: { id: user.id, name, email } } });
  } catch (err) {
    next(err);
  }
}
...
export async function login(req, res, next) {
  try {
    ...
    res.json({ success: true, data: { user: { id: user.id, name: user.name, email }, accessToken, refreshToken } });
  } catch (err) {
    next(err);
  }
}
